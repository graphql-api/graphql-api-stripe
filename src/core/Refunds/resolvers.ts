...
export const resolvers: Resolvers = {
  Query: {
    listRefunds: (_, args, context, info) =>
      context.stripe.refunds.list(...exp(args.where, info, ['data'])),
    retrieveRefund: (_, args, context, info) =>
      context.stripe.refunds.retrieve(...exp(args.where.id, info))
  },
  Mutation: {
    createRefund: (_, args, context, info) =>
      context.stripe.refunds.create(
        ...exp({ charge: args.whereCharge.id, ...args.data }, info)
      ),
    updateRefund: (_, args, context, info) =>
      context.stripe.refunds.update(args.where.id, ...exp(args.data, info))
  }
}
