...
export type ChargeCaptureDestinationInput = {
  /** The portion of this charge to send to the destination account. Must be less
   * than or equal to the captured amount of the charge.
   */
  amount: Maybe<Scalars["Int"]>;
};

export type ChargeCaptureInput = {
  /** The amount to capture, which must be less than or equal to the original
   ...
   * the item you’re charging for is a race ticket, you may want to specify a
   * statement_descriptor of RunClub 5K race ticket. The statement description must
   * contain at least one letter, may not include <>' characters, and will appear
   * on your customer’s statement in capital letters. Non-ASCII characters are
   * automatically stripped. Updating this value will overwrite the previous
   * statement descriptor of this charge. While most banks display this information
...
export type ChargeCreateDestinationInput = {
  /** ID of an existing, connected Stripe account. */
  account: Scalars["ID"];
  /** The amount to transfer to the destination account without creating an
   * Application Fee object. Cannot be combined with the application_fee parameter.
...
export type ChargeCreateInput = {
   ...
   * currency). The minimum amount is $0.50 USD or equivalent in charge currency.
   */
   ...
   */
  application_fee: Maybe<Scalars["Int"]>;
  /** Whether to immediately capture the charge. Defaults to true. When false, the
   * charge issues an authorization (or pre-authorization), and will need to be
   * captured later. Uncaptured charges expire in seven days. For more information,
   * see the authorizing charges and settling later documentation.
   */
  capture: Maybe<Scalars["Boolean"]>;
  card_as_payment_method: Maybe<Scalars["Boolean"]>;
   ...
   * include the description of the charge(s) that they are describing. This can be
   * unset by updating the value to null and then saving.
   */
  description: Maybe<Scalars["String"]>;
  metadata: Maybe<Scalars["JSON"]>;
  /** The Stripe account ID for which these funds are intended. Automatically set if
   * you use the destination parameter. For details, see Creating Separate Charges and Transfers.
   */
  on_behalf_of: Maybe<Scalars["ID"]>;
  /** The email address to which this charge’s receipt will be sent. The receipt
   * will not be sent until the charge is paid, and no receipts will be sent for
   * test mode charges. If this charge is for a Customer, the email address
  ...
  /** A payment source to be charged. This can be the ID of a card (i.e., credit or
   * debit card), a bank account, a source, a token, or a connected account. For
   * certain sources—namely, cards, bank accounts, and attached sources—you must
   * also pass the ID of the associated customer.
   */
  source: Maybe<Scalars["ID"]>;
  /** An arbitrary string to be displayed on your customer’s credit card statement.
   * This can be up to 22 characters. As an example, if your website is RunClub and
   * the item you’re charging for is a race ticket, you might want to specify a
   * statement_descriptor of RunClub 5K race ticket. The statement description must
   * contain at least one letter, must not contain <> characters, and will appear
   * on your customer’s statement in capital letters. Non-ASCII characters are
   * automatically stripped. While most banks and card issuers display this
...
export type ChargeUpdateInput = {
   ...
   * field may only be updated if there is no existing associated customer with this charge.
   */
  customer: Maybe<Scalars["ID"]>;
  /** An arbitrary string which you can attach to a charge object. It is displayed
   * when in the web interface alongside the charge. Note that if you use Stripe to
   ...
   */
  description: Maybe<Scalars["String"]>;
  fraud_details: Maybe<FraudDetailsInput>;
  /** Set of key-value pairs that you can attach to an object. This can be useful
   * for storing additional information about the object in a structured format.
   ...
   * address. This can be unset by updating the value to null and then saving.
   */
  receipt_email: Maybe<Scalars["String"]>;
  shipping: Maybe<ShippingInput>;
  /** A string that identifies this transaction as part of a group. transfer_group
...
export type ChargeWhereUniqueInput = {
  id: Scalars["ID"];
};
 ...
 * a value of fraudulent. If you believe a charge is safe, include a user_report
 * key with a value of safe. Note that you must refund a charge before setting the
 * user_report to fraudulent. Stripe will use the information you send to improve
 * our fraud detection algorithms.
 */
export type FraudDetailsInput = {
  user_report: Maybe<UserReport>;
};
...
export type Mutation = {
   ...
   * mode. (Stripe assumes that the charge would have completed successfully).
   */
...
export type MutationCaptureChargeArgs = {
  where: ChargeWhereUniqueInput;
...
export type QueryRetrieveChargeArgs = {
  where: Maybe<ChargeWhereUniqueInput>;
};
...
export type MutationResolvers<
  ...
  ParentType = Mutation
> = {
  createCharge: Resolver<
    Maybe<Charge>,
    ParentType,
    Context,
    MutationCreateChargeArgs
  >;
  updateCharge: Resolver<
    Maybe<Charge>,
    ParentType,
    Context,
    MutationUpdateChargeArgs
  >;
  captureCharge: Resolver<
    Maybe<Charge>,
    ParentType,
    Context,
    MutationCaptureChargeArgs
  >;
};
...
export type QueryResolvers<Context = StripeContext, ParentType = Query> = {
  ...
  retrieveCharge: Resolver<
    Maybe<Charge>,
    ParentType,
    Context,
    QueryRetrieveChargeArgs
  >;
  listCharges: Resolver<
    Maybe<ChargeList>,
    ParentType,
    Context,
    QueryListChargesArgs
  >;
};
