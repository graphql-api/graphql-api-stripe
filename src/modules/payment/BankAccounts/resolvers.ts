...
export const resolvers: Partial<Resolvers> = {
  Query: {
    listBankAccounts: (parent, args, context, info) =>
      context.dataSources?.stripe?.customers.listSources(
        args.whereCustomer.id,
        ...exp(args.input, info)
      ),
    retrieveBankAccount: (parent, args, context, info) =>
      context.dataSources?.stripe?.customers.retrieveSource(
        args.whereCustomer.id,
  ...
  Mutation: {
    createBankAccount: (parent, args, context, info) =>
      context.dataSources?.stripe?.customers.createSource(
        args.whereCustomer.id,
        ...exp(args.data, info)
      ),
    ...
    updateBankAccount: (parent, args, context, info) =>
      context.dataSources?.stripe?.customers.updateCard(
        args.whereCustomer.id,
        ...exp(args.data, info)
      ),
    verifyBankAccount: (parent, args, context, info) =>
      context.dataSources?.stripe?.customers.verifySource(
        args.whereCustomer.id,
        ...exp(args.data, info)
      )
  }
